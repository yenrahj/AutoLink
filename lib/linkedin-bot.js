...
const CHROMIUM_BINARY_URL = 'https://github.com/Sparticuz/chromium/releases/download/v129.0.0/chromium-v129.0.0-pack.tar';
...
class LinkedInBot {
  ...
  async initialize() {
    try {
      ...
      // Download and use the remote Chromium binary
  ...
  async login() {
    try {
      ...
      if (url.includes('/feed') || url.includes('/in/')) {
        ...
        throw new Error('LinkedIn security checkpoint detected. Please login manually first.');
      } else {
